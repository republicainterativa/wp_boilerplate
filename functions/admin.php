...
function mycustom_dashboard_widgets() {
    ...
    wp_add_dashboard_widget('custom_help_widget', 'Bem vindo ao painel, '.get_bloginfo( 'name' ), 'custom_dashboard_help');
}
...
if ( is_admin() && $current_user->user_login != '' ) {
    ...
    function remove_submenus() {
        ...
        remove_submenu_page('themes.php', 'customize.php?return=%2Flabcareer%2Fwp-admin%2F');
...
function change_admin_label_names( $translated ) {  
    $translated = str_replace( 'Aparência', 'Geral', $translated );
    $translated = str_replace( 'aparencia', 'geral', $translated );
    return $translated;
}
